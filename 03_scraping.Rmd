...
> Actividad (8 min): Buscar artículos de investigación (paper) que usen el web scraping. Fuente de datos, objetivo, método.

  - Título; La mujer en youtube, representación y participación
    + Fuente de datos: youtube
    + Objetivo: Brechas de género
    + Método: Analítica descriptiva
  - Título: La prolongación de la crisis sanitaria
    + Fuente de datos: fuentes abiertas con datos del covid
    + Objetivo: Impactos de la pandemia
    + Método: Analítica descriptiva/diagnóstica
  - Título: Aplicaciones y herramientas para el WS
    + Fuente de datos: Tiendas online
    + Objetivo: extraer los datos (xml, sql, excel)
    + Métodos: ---
...
## Librería rvest
  ...
  - Optimizada para páginas estáticas.
  - Se encuentra dentro del universo "tidyverse"
...
Como se vi antes, una herramienta para facilitar la identificación del CSS o "nodo" es el selector gadget.
...
aux<-html_nodes(covid,".maincounter-number")
html_text(aux)
html_text2(aux)
html_attrs(aux)
html_children(aux)
...
### Ejercicio: Prensa digital

A partir de la página de Urgentebo, capture los titulares. 
```{r}
www<-"https://www.urgente.bo/"
prensa<-read_html(www)
aux<-html_nodes(prensa,"h2")
html_text2(aux)

aux2<-html_nodes(prensa,".normal-image-content")
html_text2(html_nodes(aux2,"h2"))
```

### Ejemplo: Tiendas digitales

```{r}
# FIDALGA
www<-"https://www.fidalga.com/collections/panaderia-y-reposteria"
tienda<-read_html(www)

aux<-html_nodes(tienda,".grid-item-border")
html_text2(html_nodes(aux,".price-regular"))
# ultracasas

www<-"https://www.ultracasas.com/buscar/casa-en-venta--en--la-paz---la-paz?page=5"

casa<-read_html(www)
aux<-html_nodes(casa,".pos-relative")
html_text2(html_nodes(aux,".inmuebles-item-precio"))
...
download.file(aux1[7],"C:\\Users\\clases\\Desktop\\asfi\\boletin.xlsx", mode = "wb")

for(i in 1:length(aux2)){
  nombre<-paste0("C:\\Users\\clases\\Desktop\\asfi\\boletin_",i,".xlsx")
...
library(rvest)
library(xml2)
www<-"https://www.infocasas.com.bo/venta/casas/la-paz/pagina3"
####base de datos
...
Algunos ejemplos de aplicación
  ...
  + Comportamiento de las negociaciones en el mercado financiero. (Deuda)
  + Estimar la incidencia de tipo de influenza (epidemiología) 
  + Bienestar mental en el COVID-19 (salud mental)
  + Análisis de subidas de casos de COVID-19. (síntomas)
  + Detectar polen atmosférico (alergias)
...
View(bd1$interest_over_time)
plot(bd1)

bd2<-gtrends("Bolivia", time = "today 1-m")
plot(bd2)
View(bd2$interest_over_time)

bd3<-gtrends(c("Bolivia", "Eliminatorias"), time = "today 1-m", geo = "BO")
plot(bd3)
View(countries)

bd4<-gtrends(c("dolar", "inflacion","Luis Arce"), time = "today 1-m", geo = "BO")
plot(bd4)
View(bd4$related_queries)
```
...
t1<-wb_data(country = c("BOL","PER"),indicator = "NY.GDP.MKTP.CD", start_date = 1900, end_date = 2023)
