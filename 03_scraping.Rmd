...
## Fuentes de información convencionales
  ...
  - *Censos*: La principal amenaza corresponde a la sobre y sub cobertura. 
  ...
  - *Registros administrativos:* Normalmente los registros administrativos tienen problemas de contenido. Ejemplo; dataset de importaciones y exportaciones del INE del 2023/2024. 
  
> ACTIVIDAD PARA LA PRÓXIMA CLASE: Buscar y descargar los ficheros de la encuesta a hogares y las importaciones y exportaciones.  
...
library(readxl)
exp23<-read_xlsx("~/Documentos/datos/EXPORTACIONES 2023p.xlsx")
exp24<-read_excel("_data/EXPORTACIONES ENE A MAY 2024p.xlsx")
imp23<-read_xlsx("~/Documentos/datos/IMPORTACIONES_2023p.xlsx")
imp24<-read_xlsx("~/Documentos/datos/IMPORTACIONES_ENE_MAY_2024p.xlsx")
save(exp23, exp24, imp23, imp24, file = "_data/ie2324.RData")
#Windows
#C:/Documentos/datos/EXPORTACIONES 2023p.xlsx
#C:\\Documentos\\datos\\EXPORTACIONES 2023p.xlsx
rm(list=ls())
...
```{r}
#Para la encuesta a hogares
library(haven)
library(labelled)#etiquetas
eh22d<-read_sav("~/Documentos/datos/BD22/BD/EH2022_Discriminacion.sav")

aux<-dir("~/Documentos/datos/BD22/BD/")
d0<-"~/Documentos/datos/BD22/BD/"

eh22d<-read_sav(paste0(d0,aux[1]))
eh22e<-read_sav(paste0(d0,aux[2]))
eh22g<-read_sav(paste0(d0,aux[3]))
eh22p<-read_sav(paste0(d0,aux[4]))
eh22s<-read_sav(paste0(d0,aux[5]))
eh22v<-read_sav(paste0(d0,aux[6]))
save(eh22d, eh22e, eh22g, eh22p, eh22s, eh22v, file="_data/eh22.RData")
```
