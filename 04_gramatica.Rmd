...
## Recopilación
  ...
  + Puede llevar a un proceso de *recolección de datos*
...
> Ejemplo: Se quiere estudiar la pobreza y su relación con la educación en el área urbano y rural. Resp. EH-22 

> Ejemplo: Se quiere estudiar la relación entre los votos obtenidos por las dos principales fuerzas en la última elección a presidente de Bolivia. Resp: OEP.
...
# para el R los comandos de importación inician con read...
...
> Ejemplo: Exportaciones 2023, 2024  
  ...
  - **Cobertura temporal:** 2023, 2024 (mayo)
...
library(dplyr)
names(exp23)
names(exp23)<-tolower(names(exp23))#minuscula toupper(mayus)
#renombrar variables
exp23<-exp23 %>% rename(cadu=adudes, dadu=desadu)
#aux<-exp23 %>% rename(cadu=dadu)
class(exp23$cadu)
typeof(exp23$cadu)
apply(exp23, 2, typeof)
```
...
### Reportes básicos
  ...
  - *Medidas de forma:* Cuantiles (acumulación); Cuartiles, quintiles, deciles, percentiles
